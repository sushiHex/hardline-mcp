...
| `history(limit=50, agent=None)` | Recent messages newest-first; `agent` matches sender or recipient. Never acks and never hides acked messages, so it is the recovery path for anything `inbox` consumed. Capped at `MAX_HISTORY_LIMIT`; bodies truncated like `inbox`. |
